...
import { materializeProjectionBatch } from './projection-materializer.js';
import {
...
export async function claimProjectionRows(
  db,
  rows,
  nowIso = new Date().toISOString(),
  staleBeforeIso = projectionLeaseStaleBefore(Date.parse(nowIso))
) {
...
export async function processProjectionOutbox(db, options = {}) {
  ...
  const materializeRows = options.materializeRows || materializeProjectionBatch;
  ...
  const claimed = await claimRows(db, loaded, nowIso, staleBeforeIso);
  if (!claimed.length) return { ok: true, code: 'PROJECTION_CLAIM_RACE', loaded: loaded.length, claimed: 0, acked: 0, failed: 0 };

  const materialized = await materializeRows(db, claimed);
  let failed = 0;
  for (const failure of Array.isArray(materialized?.failures) ? materialized.failures : []) {
    await failRow(db, failure.row, failure.code || 'PROJECTION_MATERIALIZATION_FAILED', nowMs);
    failed += 1;
  }
  const sendable = Array.isArray(materialized?.rows) ? materialized.rows : [];
  if (!sendable.length) {
    return {
      ok: false,
      code: 'PROJECTION_BATCH_INCOMPLETE',
  ...
  let delivery;
  try {
    delivery = await sendBatch(sendable, {
    ...
    const code = String(error?.message || 'PROJECTION_DELIVERY_FAILED').slice(0, 128);
