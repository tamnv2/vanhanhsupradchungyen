...
public sealed class LanClientSecurityStore
{
    ...
    public async Task<LanClientSecurityInspection> InspectAsync(CancellationToken cancellationToken = default)
    {
        ...
        state.CommandText = "SELECT security_epoch FROM lan_security_state WHERE singleton_id=1 LIMIT 1";
        var epoch = Convert.ToString(await state.ExecuteScalarAsync(cancellationToken));
        if (string.IsNullOrWhiteSpace(epoch))
        {
            return new LanClientSecurityInspection(false, SecuritySchemaVersion, null, 0, 0, 0, "SECURITY_STATE_REQUIRED");
        }
        ...
        return new LanClientSecurityInspection(true, SecuritySchemaVersion, epoch, active, repair, revoked, "READY");
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        read.CommandText = """
            SELECT public_key_spki_base64, status, paired_security_epoch
            FROM lan_paired_devices
        ...
        if (existingStatus == "REVOKED")
        {
            ...
            throw new LanClientSecurityException("PAIRING_REVOKED_DEVICE", "A revoked device identity requires an explicit new device identity before pairing.");
        }
        ...
        {
            transaction.Rollback();
            throw new LanClientSecurityException("PAIRING_CONFLICT", "An active device identity is already paired with different security evidence.");
        }
        ...
        write.CommandText = """
            INSERT INTO lan_paired_devices(
              ...
              paired_by_user_id, paired_security_epoch, paired_at, updated_at
            ) VALUES (
              $deviceId, $publicKey, $algorithm, 'ACTIVE',
              $actor, $epoch, $now, $now
            )
        ...
        transaction.Commit();
        return new LanPairingResult(deviceId, epoch, "ACTIVE", SignatureAlgorithm);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = """
            UPDATE lan_paired_devices
            SET status='REVOKED', revoked_at=$now, updated_at=$now
            WHERE device_id=$deviceId AND status <> 'REVOKED'
            """;
    ...
    public async Task<string> RotateSecurityEpochAsync(
        string actorUserId,
        string reason,
        CancellationToken cancellationToken = default)
    {
        ...
        await using (var updateState = connection.CreateCommand())
        {
            ...
            updateState.CommandText = """
                UPDATE lan_security_state
                SET security_epoch=$epoch, rotated_at=$now, updated_at=$now
                WHERE singleton_id=1
                """;
            updateState.Parameters.AddWithValue("$epoch", nextEpoch);
            updateState.Parameters.AddWithValue("$now", now);
        ...
        await using (var repair = connection.CreateCommand())
        {
            ...
            repair.CommandText = """
                UPDATE lan_paired_devices
                SET status='REPAIR_REQUIRED', updated_at=$now
                WHERE status='ACTIVE'
                """;
            repair.Parameters.AddWithValue("$now", now);
        ...
        await AppendAuditAsync(
            connection,
            transaction,
            "SECURITY_EPOCH_ROTATED",
            actorUserId,
            null,
            nextEpoch,
            $"{reason}; previousEpoch={oldEpoch}",
            cancellationToken);
        transaction.Commit();
        return nextEpoch;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var sentAt = DateTimeOffset.FromUnixTimeMilliseconds(proof.TimestampUnixMs);
        ...
        {
            return Deny("SECURITY_STATE_REQUIRED", proof.DeviceId, null);
        }
        ...
        if (device is null)
        {
            return Deny("DEVICE_NOT_PAIRED", proof.DeviceId, currentEpoch);
        }
        ...
        if (!string.Equals(device.SignatureAlgorithm, SignatureAlgorithm, StringComparison.Ordinal))
        {
        ...
        try
        {
            ...
            nonce.CommandText = """
                INSERT INTO lan_request_nonces(
                  device_id, security_epoch, nonce, request_digest, accepted_at
                ) VALUES (
                  $deviceId, $epoch, $nonce, $digest, $acceptedAt
                )
                """;
        ...
        return new LanClientSecurityDecision(true, "CHANNEL_AUTHORIZED", proof.DeviceId, currentEpoch);
    }
    ...
    private static async Task<string> EnsureStateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        insert.CommandText = """
            INSERT INTO lan_security_state(singleton_id, schema_version, security_epoch, created_at, rotated_at, updated_at)
            VALUES (1, $schema, $epoch, $now, NULL, $now)
            """;
    ...
    {
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT security_epoch FROM lan_security_state WHERE singleton_id=1 LIMIT 1";
        return Convert.ToString(await command.ExecuteScalarAsync(cancellationToken));
    }

    private static async Task<PairedDevice?> ReadDeviceAsync(
        SqliteConnection connection,
        string deviceId,
        CancellationToken cancellationToken)
    {
        await using var command = connection.CreateCommand();
        command.CommandText = """
            SELECT public_key_spki_base64, signature_algorithm, status, paired_security_epoch
            FROM lan_paired_devices
            WHERE device_id=$deviceId
            LIMIT 1
            """;
        ...
        return new PairedDevice(reader.GetString(0), reader.GetString(1), reader.GetString(2), reader.GetString(3));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        command.Transaction = transaction;
        command.CommandText = """
            INSERT INTO lan_security_audit(
              audit_event_id, event_type, actor_user_id, device_id,
              security_epoch, reason, occurred_at
            ) VALUES (
              $eventId, $eventType, $actor, $deviceId,
              $epoch, $reason, $occurredAt
            )
            """;
    ...
    private static void ValidateP256PublicKey(string publicKeySpkiBase64)
    {
        try
        {
            ...
            if (read != bytes.Length || ecdsa.KeySize != 256)
                throw new LanClientSecurityException("PUBLIC_KEY_INVALID", "Paired client key must be an ECDSA P-256 SubjectPublicKeyInfo key.");
        }
        catch (LanClientSecurityException)
        {
            throw;
        }
    ...
    private static bool VerifySignature(string publicKeySpkiBase64, string canonical, string signatureBase64)
    {
        try
        {
            ...
            if (read != key.Length || ecdsa.KeySize != 256) return false;
    ...
    private static void ValidateProof(LanClientSignedRequestProof proof, bool requireSignature = true)
    {
        ...
        if (!IsLowerHexSha256(proof.BodySha256))
            throw new LanClientSecurityException("BODY_HASH_INVALID", "Signed body hash must be a 64-character SHA-256 hex string.");
    ...
    private static LanClientSecurityDecision Deny(string code, string deviceId, string? epoch) =>
        new(false, code, deviceId, epoch);

    private sealed record PairedDevice(
        string PublicKeySpkiBase64,
        string SignatureAlgorithm,
        string Status,
        string PairedSecurityEpoch);
...
CREATE INDEX IF NOT EXISTS ix_lan_paired_devices_status
ON lan_paired_devices(status, updated_at);
...
CREATE TRIGGER IF NOT EXISTS trg_lan_security_audit_no_update
BEFORE UPDATE ON lan_security_audit
BEGIN SELECT RAISE(ABORT, 'lan security audit is immutable'); END;

CREATE TRIGGER IF NOT EXISTS trg_lan_security_audit_no_delete
BEFORE DELETE ON lan_security_audit
BEGIN SELECT RAISE(ABORT, 'lan security audit is immutable'); END;
""";
}
...
public sealed record LanClientSecurityDecision(
    bool Authorized,
    string Code,
    string DeviceId,
    string? SecurityEpoch);

public sealed record LanClientSecurityInspection(
    bool Ready,
    string SchemaVersion,
    string? SecurityEpoch,
    long ActiveDeviceCount,
    long RepairRequiredDeviceCount,
    long RevokedDeviceCount,
    string Code);

public sealed record LanPairingResult(
    string DeviceId,
    string SecurityEpoch,
    string Status,
    string SignatureAlgorithm);
